...
Multiple Epics were fully implemented. Some exceed expectations with advanced features, and exceptional value addition to the app.
Requirements are seamlessly integrated, adding substantial value to the app. All requirements are essential to the app and greatly come together.
All features are fully implemented, but some might feel a bit empty (e.g., the planet description screen). All implementations are bug-free at first sight, and thoughtfully designed.
...
Some loading times are a tad long, otherwise, LGTM
...
App offers good usability, and is quite intuitive.
...
No dependency injection for `FirebaseAuth`.
It will be super tricky if you ever want to switch the authentication mechanism.
Side note regarding the commit messages: this might sound like detail, but some of you start all their commit messages with a capital letter, while the others don't. It's one thing to be consistent across your own PRs, but consistency between all of you should be enforced as well.
...
Test suit is documented, well organized and test coverage is great.
...
The "LOOKUP DOCUMENTATION" is nice but lacks technical documentation that would help another developer to quickly take on the project.
Architecture diagram is complete and up-to-date with the current state of the app. However, "ViewModel layer" is not a thing. There are 3 layers in MVVM, the UI layer, the domain layer, and the data layer. Repositories are part of the data layer, while screens and view-models are part of the UI layer. The domain layer is optional.
...
Demo was not always properly prepared, but there was clear improvements on that side by the end of the milestone
...
Based on the above points, your intermediate grade for this milestone M3 is 5.56.
