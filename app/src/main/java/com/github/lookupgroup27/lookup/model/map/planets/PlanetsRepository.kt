...
class PlanetsRepository(
    ...
    fun updatePlanetsData() {
        val currentLocation = locationProvider.currentLocation.value
            ?: throw IllegalStateException("Current location is null. Cannot update planetary data.")
    ...
    private fun fetchPlanetPosition(lat: Double, lon: Double, planetId: String): Pair<Double, Double>? {
        ...
        client.newCall(request).execute().use { response ->
            ...
            val dataLine = dataLines.firstOrNull { it.isNotBlank() && !it.startsWith("Date") } ?: return null
