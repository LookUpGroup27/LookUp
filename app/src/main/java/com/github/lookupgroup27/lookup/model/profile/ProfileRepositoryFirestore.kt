...
class ProfileRepositoryFirestore(
    ...
    override fun updateUserProfile(
        ...
        if (userId != null) {
            ...
            usersCollection.whereEqualTo("username", profile.username).get()
                .addOnSuccessListener { querySnapshot ->
                    val isUsernameTaken = querySnapshot.documents.any { it.id != userId }
                    if (isUsernameTaken) {
                        onFailure(UsernameAlreadyExistsException("Username '${profile.username}' is already in use."))
                    } else {
                        // Proceed to update the user profile
                        usersCollection.document(userId).set(profile)
                            .addOnSuccessListener { onSuccess() }
                            .addOnFailureListener { exception -> onFailure(exception) }
                    }
                }
                .addOnFailureListener { exception ->
                    Log.e("ProfileRepositoryFirestore", "Error checking username uniqueness", exception)
                    onFailure(exception)
                }
        } else {
