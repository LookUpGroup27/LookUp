 ...
 * - Fetches current planetary positions (Right Ascension and Declination) from NASA/JPL Horizons API.
 * - Converts the obtained RA/Dec into horizon coordinates (Azimuth, Altitude), and then into Cartesian coordinates
...
class PlanetsRepository(
    ...
    init {
        ...
        // Each planet has a unique Horizons ID, and we currently associate all planets with the same texture.
        planets.addAll(
            listOf(
     ...
     * 2. Compute the local sidereal time for that location (this aligns the celestial coordinate system with local time).
     * 3. For each planet:
     ...
     * @return A Pair<Double, Double>? representing (RA in degrees, Dec in degrees) or null if unavailable.
     */
    private fun fetchPlanetPosition(
        ...
        client.newCall(request).execute().use { response ->
            ...
            // According to Horizons format, RA and Dec are typically in columns 4 and 5 (0-based index: 3 and 4)
            if (columns.size >= 3) {
